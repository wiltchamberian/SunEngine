...
//虽然是一个简单的createBuffer，其实有很多细节可以理解
/*
* 输入:bufferCreateInfo: 创建buffer需要的一些信息，和buffer用途有关
* memProperties_ :这个buffer需要绑定内存，因此需要创建内存，则创建的内存需要指定属性（设备内存，主机内存之类的)
* buffer只是一个句柄，指向一个结构体，这个结构体需要绑定到内存,
* Vulkan的内存是单独申请的一大块数据， 每个buffer只绑定到这一大块数据中间的一小段，
* 因此内存的申请放在buffer中就不合适（因为memory和buffer是一对多关系)
* 所以需要单独的内存管理来申请内存，并管理维护这块内存和buffer的关系
* nvvk将这部分工作做了
* 对于buffer来说，绑定内存时显然需要指定三个参数，1内存句柄，2偏移量，3长度
*/
Buffer ResourceAllocator::createBuffer(const VkBufferCreateInfo& info_, const VkMemoryPropertyFlags memProperties_)
{
  ...
  //调用vkCreateBuffer，创建一个buffer(这个buffer还没绑定内存),但这个buffer已经知道
  //它需要的内存需求
  CreateBufferEx(info_, &resultBuffer.buffer);

  //为了给buffer绑定内存，我们需要知道这个buffer需要的内存需求是什么
  //利用buffer句柄，和Vulkan的API,我们来获取这个buffer需要的内存需求
  //内存需求主要包含3个东西:内存类型，内存长度，和内存对齐
  //[注:bufferCreateInfo里只是包含了buffer类型，这个类型和buffer内存需求存在一个对应关系，这个对应关系Vulkan知道
  //因此，我们利用Vulkan API来获取该buffer对应的内存需求]
  
  // Find memory requirements 
  //memReqs也就是追踪需要得到内存类型，这里叫内存需求
  ...
  //有了内存需求和，内存属性，交给nvvk的内存分配器来分配内存
  ...
  // Bind memory to buffer （nvvk已经把所有内存都管理好了，返回bubffer需要的句柄
  //内存偏移量，和长度，直接绑定即可

...
//use implicit surface to descrip the object
//we trans the impicit surface to a DAG struct which is an array of tokens
struct Token{
  int opcode;
  float real;
  int left;
...
const int numSurface = 20;
//signed distance field
layout(std430, binding =3) buffer ExpressionBuf{
  Expression sdfs[];
};
...
float cal_sdf(inout Expression sdf, vec3 p){
  for(int i=0;i<sdf.tokennum;++i)
  {
    switch (dag.code) {
      case OP_FLOAT: 
      {
        sdf.tokens[i].value = sdf.tokens[i].real;
      }
      break;
      case OP_X:
      {
        sdf.tokens[i].value = p.x;
      }
      break;
      case OP_Y:
      {
        sdf.tokens[i].value = p.y;
      }
      break;
      case OP_Z:
      {
        sdf.tokens[i].value = p.z;
      }
      break;
      case OP_SYM:
      {
      ...
      case OP_ABS:
      {
        sdf.tokens[i].value = abs(sdf.tokens[sdf.tokens[i].left].value);
      }
      break;
      case OP_SIN:
      {
        sdf.tokens[i].value = sin(sdf.tokens[sdf.tokens[i].left].value);
      }
      break;
      case OP_COS:
      {
        sdf.tokens[i].value = cos(sdf.tokens[sdf.tokens[i].left].value);
      }
      break;
      case OP_SQRT:
      {
        sdf.tokens[i].value = sqrt(sdf.tokens[dag.left].value);
      }
      break;
      case OP_ADD:
      {
        sdf.tokens[i].value = sdf.tokens[sdf.tokens[i].left].value + sdf.tokens[sdf.tokens[i].right].value;
      }
      break;
      case OP_SUB:
      {
      ...
      case OP_POW:
      {
        sdf.tokens[i].value = std::pow(sdf.tokens[sdf.tokens[i].left].value, sdf.tokens[sdf.tokens[i].right].value);
      }
      break;
      default:
        break;
  ...
  return std.tokens[sdf.tokennum-1].value;
}

float cal_sdf_all(vec3 p,inout id){
  float res = 1000000.0;
  int k= 0;
  for(int i=0;i<sdfs.length();++i){
    float tmp = cal_sdf(sdfs[i],p);
    if(tmp<res){
      k = i;
    }
    res = min(tmp,res);
...
int hit_implicit_surface(int id, Ray ray, float t_min, float t_max,inout HitRecord rec){
  return 0;
...
int rayMarchingOnHit(in Ray ray, inout HitRecord rec){
  float t = 0.001;
  vec3 p = ray.origin + ray.direction*t;
  const int NUM_SETPS = 50;
  for(int i=0;i<NUM_SETPS;++i){
    vec3 p = ray.origin+ray.direction*t;
    int id = 0;
    float dis = cal_sdf_all(p,id);
  }
